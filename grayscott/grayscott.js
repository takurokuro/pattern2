...
// Configuration.
var preset_init = 2;
var feed = presets[preset_init].feed;
var kill = presets[preset_init].kill;

// ccapture
var capturer = new CCapture({
    // format: 'gif', workersPath: '../libraries/',
    format: 'png',
...
init = function()
{
    ...
    mUniforms = {
        ...
        brush2: {type: "v2", value: new THREE.Vector2(-10, -10)},
    ...
    //個数、初期座標設定
    render(0);
    mUniforms.brush.value = new THREE.Vector2(0.5, 0.5); //初期位置（x座標：0.0~1.0, y座標：0.0~1.0）
    
    ランダムに発生
    for(var i = 0; i < 25; i++) {
        render(0);
        mUniforms.brush.value = new THREE.Vector2(Math.random(), Math.random()); //初期位置（x座標：0.0~1.0, y座標：0.0~1.0）
    };
